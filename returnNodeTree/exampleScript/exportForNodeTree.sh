...
function checkCLArgumentValue() {
	#check if the deployable name was provided as input. If not ask for it	
	if [ ! -z $1 ] 
	then 
	    argValue=$1 
	else
	    read -p "enter the value for the argument to be used for the export: " argValue; echo " "
	fi
	echo -e "this script will run with argument value \e[92m${argValue}\e[39m"
}
...
checkCLArgumentValue $5
#appName="app-demo-1615796194"
#deployableName="PRD"
expName="${expName}&args=%7B%22nodeName%22%3A%22${argValue}%22%7D"
